...
<p align="center"><img src="assets/rmse_comparison.png" width="380" alt="2025 vs 2026 Test RMSE 비교"></p>
...
> **※ "스펙을 알면 성능도 알지 않나?"에 대해**: 스펙이 성능을 어느 정도 결정하는 건 사실이지만, 실제로는 (1) 출시 전 스펙 시트만 공개되고 실측 벤치마크는 나중에 나오는 경우가 많고, (2) 같은 스펙이라도 드라이버·아키텍처 최적화·소프트웨어 스택 차이로 실측 점수가 갈린다. 이 프로젝트는 "스펙만으로 벤치마크를 100% 재현"하는 게 목표가 아니라, **공개 스펙이 실측 성능 분산의 어느 정도를 설명하는지, 그리고 스펙만으로는 설명 안 되는 나머지(Architecture 세대 효과 등)가 무엇인지**를 회귀 계수로 뜯어보는 해석 중심 분석에 가깝다.
...
→ 2025는 "접두어가 같으면 매칭"하는 방식을 썼는데, 이 방식은 RTX 3060이 RTX 3060 Ti 스펙과 잘못 묶이는 등 **오매칭 위험**이 있었다. 2026은 이 방식을 버리고 정확히 일치하는 경우만 매칭했는데도(오히려 더 엄격한 조건인데도) 이름 정제 로직 자체를 개선한 덕에 매칭 수는 385개 → 447개로 **더 늘었다**. 즉 매칭 기준을 깐깐하게 하면서도 매칭 수는 늘어난 것이라, 늘어난 매칭 전부가 오매칭 없이 정확히 검증된 상태다.
...
<p align="center"><img src="assets/architecture_boxplot.png" width="480" alt="아키텍처 세대별 log_Score 박스플롯"></p>
...
<p align="center"><img src="assets/regression_coefficients.png" width="520" alt="최종 회귀 계수"></p>
...
>
> **※ 재현성 참고**: 위 표는 실제 로컬 실행 결과다. 다른 환경(패키지 버전 등)에서 노트북을 재실행하면 GridSearchCV가 고르는 최적 하이퍼파라미터가 살짝 달라질 수 있어 XGBoost (tuned) 행의 Train/Test R²는 환경에 따라 다소 흔들릴 수 있음을 확인했다 (검증 중 재실행 시 Train R² 0.99 근처까지 더 오버피팅되는 조합이 나온 적 있음). 다만 "Linear Regression의 CV R²가 튜닝된 XGBoost와 비슷하거나 오히려 근소 우위"라는 핵심 결론은 재실행에서도 동일하게 재현됐다.

<p align="center"><img src="assets/predicted_vs_actual.png" width="480" alt="Linear Regression 예측값 vs 실제값 (Test set)"></p>
